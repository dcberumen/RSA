...
def getFileSig(fileName, privKey):
    ...
    signiture = digSig(privKey, dataHash)
    return signiture
...
def verifyFileSig(fileName, pubKey, signature):
    ...
    f.close()
    dataHash = SHA512.new(data).hexdigest()
    verify = verifySig(dataHash, signature, pubKey)
    return verify
...
def main():
    ...
    if mode == "sign":
        ...
        print "Signature of {} saved to file {}".format(inputFileName, sigFileName)
    ...
    # We are verifying the signature
	    ...
	    verify = verifyFileSig(inputFileName, key, signature)
	    if verify == True:
	        print "Match!"
	    else:
	        print "Do Not Match!"
